...
tmp = strcat('.\res_prof\EDFres50.mat');
load(tmp)
PsIN = Ps0;
...
set(AZ(1),'Fontsize',fs,'FontName','times','YColor',Color,'YTick',[-100: 10: 100]);
set(AZ(2),'Fontsize',fs,'FontName','times','YColor',Color2,'YTick',[-100: 10: 100]);
set(AZ(1),'box','off')
...
plot(Lz,Ap_z*1e3,Color,Lz,Am_z*1e3,Color2,'LineWidth',1.5);
set(gca,'Fontsize',fs,'FontName','times','XColor','k','YColor','k');
grid on;
xlabel ('length (m)');
...
%Tracé de la Puissance signal et pompe en fonction de la longueur de la fibre

subplot(3,1,2) 
Color='b';
Color2='r';
%[AZ,H1,H2]=plotyy(Lz,10*log10(Ps*1e3),Lz,10*log10(Pp*1e3));
[AZ,H1,H2]=plotyy(Lz,Ps*1e3,Lz,Pp*1e3);
...
subplot(3,1,3) 
plot(Lz,N2*(1e-25),Color,Lz,N*(1e-25),Color2,'LineWidth',1.5);
set(gca,'Fontsize',fs,'FontName','times','XColor','k','YColor','k');
grid on;
xlabel ('length (m)');
...
hold on;
plot(lbd_s*1e9,10*log10(PsL(1,jj)*1e3),'*', 'Color', 'k');
set(gca,'Fontsize',fs,'FontName','times','XColor','k','YColor','k');
grid on;
xlabel ('lambda (nm)');
...
figure(5)
plot(Lz, [0,Pp(1,1:ndz-1) - Pp(1,2:ndz)]*1000,'k')
