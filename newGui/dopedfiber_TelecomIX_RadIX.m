...
% =========================================================================
% Name : dopedfiber_RadIX_TelecomIX
% Author : Arnaud Fernandez
% Version du : 02/06/2020
%
...
%         variation=1;
%       else
%         variation=100*(abs(PsL(1,jj)-PsL(1,jj-1)))/PsL(1,jj);
%       end
%       if jj>5*ceil(Trise/dt) && variation<=tol
%              count=count+1;
%       end          
      if tt(1,jj)>tth % s   
        dPdt=abs(PsL(1,jj)-PsL(1,jj-delinc))/(abs(tt(1,jj)-tt(1,jj-delinc))*1e9);  % W/ns
          if dPdt<tol
             count=count+1;
          end   
      end  
...
%save('test119.mat');
tmp='';
tmp=strcat('.\res\EDFres',int2str(sim_nb));
save(tmp);
